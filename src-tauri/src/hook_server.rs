...
const OPENCODE_PLUGIN: &str = r#"// Helm bridge — auto-generated by Helm. Forwards every opencode event to Helm's
// localhost hook receiver so the dashboard updates instantly (status / todos /
// activity / agent / model). Tags each event with the project directory so Helm
// can route it to the right session. Never throws into opencode.
export const HelmBridge = async ({ directory, worktree }) => {
  const dir = worktree || directory || process.cwd();
  return {
    event: async ({ event }) => {
      try {
        await fetch("http://127.0.0.1:__PORT__/hook", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ directory: dir, ...event }),
        });
      } catch {}
    },
  };
...
/// Drop a tiny opencode plugin that POSTs every event to our receiver, port baked
/// in. opencode auto-loads any *.js in its global plugin/ dir, so no opencode.json
/// edit is needed. Idempotent — re-running just rewrites it with the current port.
pub fn register_opencode(port: u16) {
    let dir = opencode_plugin_dir();
    if std::fs::create_dir_all(&dir).is_err() {
        return;
    }
    let body = OPENCODE_PLUGIN.replace("__PORT__", &port.to_string());
    let _ = std::fs::write(dir.join("helm-bridge.js"), body);
}
